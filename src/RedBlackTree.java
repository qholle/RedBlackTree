...
public class RedBlackTree<T extends Comparable<T>> implements SortedCollectionInterface<T> {
  ...
  /**
   * Recursive helper method that recurses through the tree and enforces RBT properties
   * 
   * @param newRedNode the red node to be added to the tree
   */
  private void enforceRBTreePropertiesAfterInsert(Node<T> newRedNode) {
    //base case, return and make sure root node is black.
    if (newRedNode == root || newRedNode.parent == root) {
      this.root.isBlack = true;
      return;
    }
    if (!newRedNode.parent.isBlack) { // red node with red parent violation
      if (newRedNode.parent.isLeftChild()) { // parent is left child

        if (newRedNode.parent.parent.rightChild == null
            || newRedNode.parent.parent.rightChild.isBlack) {
          if (newRedNode.isLeftChild()) { // case one violation
            this.rotate(newRedNode.parent, newRedNode.parent.parent);
            newRedNode.parent.isBlack = true;
            newRedNode.parent.parent.isBlack = false;
          } else { // case two violation
            this.rotate(newRedNode, newRedNode.parent);
            this.rotate(newRedNode, newRedNode.parent);
            newRedNode.isBlack = true;
            newRedNode.rightChild.isBlack = false;
          }
          this.root.isBlack = true;
          return;
        }

        // case three violation
        if (newRedNode.parent.parent.rightChild != null
            && newRedNode.parent.parent.rightChild.isBlack == false) {
          newRedNode.parent.parent.leftChild.isBlack = true;
          newRedNode.parent.parent.rightChild.isBlack = true;
          newRedNode.parent.parent.isBlack = false;
          // call again to check for more violations
        ...
        // case three violation
        if (newRedNode.parent.parent.leftChild != null
            && newRedNode.parent.parent.leftChild.isBlack == false) {
          newRedNode.parent.parent.leftChild.isBlack = true;
          newRedNode.parent.parent.rightChild.isBlack = true;
          newRedNode.parent.parent.isBlack = false;
          // call again to check for more violations
